...
import com.yichou.common.utils.LogUtils2;
...
public class AppProcessManager {
	public static final LogUtils2 log = LogUtils2.create("AppProcManager");
	...
	private void markAsWaiting(int index, String mark, ServiceConnection c) {
		synchronized (mProcList) {
			...
			mProcList[index].mark = mark;
			mProcList[index].readyTime = System.currentTimeMillis();
			mProcList[index].state = ItemState.WAITING;
			
			log.i("proc" + index + " wait service connection!");
		}
	}
	...
	public synchronized void requestIdleProcess(int defProcIndex, boolean foce, String mark, RequestCallback cb) {
		...
		markAsWaiting(procIndex, mark, connection);
	}
